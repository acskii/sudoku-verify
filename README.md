...
- [Ahmed Sherif](https://github.com/ahmed-sherif-hassona) - ID **9631**```mermaid
classDiagram
    class Verifier {
        +main(String[] args) void
        -readCsv(String filepath) int[][]
        +String DELIMITER
    }

    class Executor {
        +start(int[][] board, String mode) void
        -runTasks(List~Callable~List~String~~~ tasks, int poolSize) List~String~
    }

    class Worker {
        +call() List~String~
        -extractValues(int idx) int[]
        -findDuplicates(int[] values) Map~int, List~int~~
        -checkSection(int idx) List~String~
        -formatViolation(int idx, int val, List~int~ pos) String
        -int[][] board
        -WorkerType type
        -int index
        <<enum>> WorkerType (ROW, COLUMN, BOX)
        +Worker(int[][] board, WorkerType type)
        +Worker(int[][] board, WorkerType type, int index)
        ..implements.. Callable~List~String~~
    }

    Verifier --> Executor : uses
    Executor --> Worker : creates & submits tasks
  ...
  <diagram name="Page-1" id="QrykTLbAILmdaH6EqYh2">
